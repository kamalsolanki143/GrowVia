...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Wait for the Cloudflare Turnstile script (loaded in layout.tsx) to be ready
    const renderWidget = () => {
      if (
    ...
    if ((window as any).turnstile) {
      ...
      // Poll until script is ready (max ~3s)
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signInWithPassword({
      ...
      password,
      options: {
        // Pass Turnstile token — Supabase validates it server-side
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12">
        ...
        >
          ...
          {/* Hidden Turnstile container — renders invisible widget */}
          ...
          <button
            ...
            className="w-full h-11 mt-3 rounded-lg border border-border bg-card hover:bg-muted flex items-center justify-center gap-3 text-sm font-medium transition-colors disabled:opacity-50"
          >
            {isGithubLoading ? (
              <div className="h-4 w-4 border-2 border-muted-foreground/30 border-t-muted-foreground rounded-full animate-spin" />
            ) : (
